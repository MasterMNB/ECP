		...
		if(isset($_POST['name'])) {
			...
			if($_POST['name'] == '' OR $_POST['username'] == '' OR $_POST['age'] == '' OR $_POST['comment'] == '' OR !(int)$_POST['age'] OR $_POST['email'] == '' OR $_POST['country'] == '' OR !$_POST['teamID'] OR !check_email($_POST['email']) OR $_POST['captcha']== '') {
				...
				$sql = sprintf('INSERT INTO '.DB_PRE.'ecp_joinus (`name`, `username`, `email`, `icq`, `msn`, `age`, `country`, `teamID`, `comment`, `IP`, `datum`) VALUES (\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',%d,\'%s\',%d,\'%s\',\'%s\',%d)', 
														strsave(charhtmlconvert($_POST['name'])), strsave(charhtmlconvert($_POST['username'])),strsave($_POST['email']),strsave(charhtmlconvert($_POST['icq'])),
														strsave(charhtmlconvert($_POST['msn'])),(int)$_POST['age'],strsave(charhtmlconvert($_POST['country'])),(int)$_POST['teamID'],
				...
				if($db->query($sql)) {
					...
					while($row = mysql_fetch_assoc($result)) {
						...
						$replace = array(strsave($row['username']), strsave(charhtmlconvert($_POST['username'])), $id);
