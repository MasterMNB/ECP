...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_software_link($type, $ip, $c_port, $q_port, $s_port)
  {
    ...
    "bfbc2"         => "http://www.battlefieldbadcompany2.com",
    ...
    "shatteredh"    => "http://www.shatteredhorizon.com",
...
//  FORMAT RESPONSE
    ...
    else
    {
      ...
      if (strpos($request, "p") === FALSE && !$server['p'] && $server['s']['players'] != 0) { unset($server['p']); }
      if (strpos($request, "p") === FALSE && !$server['t'])                                 { unset($server['t']); }
      if (strpos($request, "e") === FALSE && !$server['e'])                                 { unset($server['e']); }
      if (strpos($request, "s") === FALSE && !$server['s']['name'] && !$server['s']['map']) { unset($server['s']); }
    }
...
//---------------------------------------------------------+
    ...
    if ($server['p'])
    {
      ...
      // RE-INDEX PLAYER KEYS TO REMOVE ANY ID GAPS
      else { $server['p'] = array_values($server['p']); }
...
//  SEPERATE TEAM DATA FROM PLAYERS DATA
    ...
    if (strpos($buffer, "\x02") !== FALSE)
    {
      $part   = explode("\x02", $buffer, 2);
      $buffer = $part[0];

      preg_match_all("/(.*)\\x00\\x00(.*)\\x00\\x00/Us", $part[1], $match);
...
//--------- PLEASE MAKE A DONATION OR SIGN THE GUESTBOOK AT GREYCUBE.COM IF YOU REMOVE THIS CREDIT -----------+
  ...
  {
    return "LGSL 5.8 Beta 8 By Richard Perry";
  }
