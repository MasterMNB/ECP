<?php
/**
  * PHP-Class hn_captcha Version 1.5.0, released 24-Dec-2007
  *
  * Version for PHP 4 !
  *
  * Author: Horst Nogajski, coding@nogajski.de
  *
  * $Id: hn_captcha.class.php4,v 1.2 2007/12/23 23:20:10 horst Exp $
  *
...
class hn_captcha
{
    ...
    //
        ...
        function make_captcha()
        {
            ...
            $_SESSION[$this->captcha_mode] = $private_key;
...
$array = array(
            'tempfolder'     => 'uploads/',      // string: absolute path (with trailing slash!) to a writeable tempfolder which is also accessible via HTTP!
			'TTF_folder'     => 'inc/ttf/', // string: absolute path (with trailing slash!) to folder which contains your TrueType-Fontfiles.
                                // mixed (array or string): basename(s) of TrueType-Fontfiles
			'TTF_RANGE'      => $fonts,
            'chars'          => ((int)$_GET['chars'] > 8  OR (int)$_GET['chars'] < 1) ? 5 : (int)$_GET['chars'],       // integer: number of chars to use for ID
            'minsize'        => ((int)$_GET['minsize'] > 30  OR (int)$_GET['minsize'] < 8) ? 8 : (int)$_GET['minsize'],      // integer: minimal size of chars
            'maxsize'        => ((int)$_GET['maxsize'] > 60  OR (int)$_GET['maxsize'] < 14) ? 14 : (int)$_GET['maxsize'],
            'captcha_mode'   => (isset($_GET['mode']) AND $_GET['mode'] == 'mini') ? 'captcha_mini' : 'captcha'   // integer: maximal size of chars			
			);
